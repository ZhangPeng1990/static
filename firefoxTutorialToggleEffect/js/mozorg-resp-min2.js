...
function init_lang_switcher() {
	$("#language").change(function(a) {
		a.preventDefault();
		gaTrack(["_trackEvent", "Language Switcher", "change", $(this).val()],
		function() {
			$("#lang_form").submit()
		})
	})
...
function init_platform_imgs() {
	...
	$(".platform-img").each(function() {
		...
		if (a(g, "osx") || a(g, "oldmac")) {
			k = "-mac"
		} else {
			if (a(g, site.platform)) {
				k = "-" + site.platform
			}
		}
		var e = b.data("src");
...
function isFirefox(b) {
	...
	return ((/\sFirefox/).test(a) && !(/like Firefox/i).test(a))
}
...
function gaTrack(a, e) {
	...
	if (typeof(window._gaq) === "object") {
		if (c) {
			d = setTimeout(b, 500);
			window._gaq.push(["_set", "hitCallback", b])
		}
		window._gaq.push(a)
	} else {
		if (c) {
	...
	a.enterSmallMode = function() {
		...
		$(document).click(a.handleDocumentClick);
		$("a, input, textarea, button, :focus").focus(a.handleDocumentFocus);
	...
	a.leaveSmallMode = function() {
		...
		$(document).unbind("click", a.handleDocumentClick);
		$("a, input, textarea, button, :focus").unbind("focus", a.handleDocumentFocus);
	...
	a.linkMenuHideOnClick = function() {
		if (a.mainMenuItems.length === 0) {
			$("#nav-main-menu > li > a").on("click.smallmode",
			function(b) {
				a.closeSmallMenu()
			})
		} else {
			$(".submenu > li > a").on("click.smallmode",
			function(b) {
				a.closeSmallMenu()
			})
		}
	...
	a.openSmallMenu = function() {
		...
		a.mainMenuLinks.addClass("submenu-item").click(a.handleSubmenuClick);
...
$(document).ready(function() {
	var e = $("#footer-email-form input[type=submit], #help-form input[type=submit]");
	var d = $("#form-details");
	function b(f) {
		$("#form-details").slideDown("normal",
		function() {
			if (f) {
				e.trigger("click")
			}
	...
	function c(g) {
		var h = $("#id_email").val();
		var f = $("#id_privacy");
		return a(h) && f.is(":checked")
	}
	$(".newsletter-form").submit(function(g) {
		var f = $(this);
		if (("checkValidity" in f) || c(f)) {
			var i = f.children("input[name=newsletter]");
			var h;
			if (i.length === 0) {
				h = "Registered for Firefox Updates"
			} else {
				h = i.val()
			}
			if (typeof(gaTrack) === "function" && h !== "") {
				g.preventDefault();
				f.unbind("submit");
				gaTrack(["_trackEvent", "Newsletter Registration", "submit", h],
				function() {
					f.submit()
				})
			}
...
Mozilla.ImageHelper.initPlatformImages = function() {
	$(".platform-img").each(function() {
		...
		if (site.platform === "osx" || site.platform === "oldmac") {
			...
			if (site.platform === "linux") {
				f = "-linux"
			}
		}
		var a = c.data("src");
		var b = a.lastIndexOf(".");
		var e = a.substring(0, b);
		var d = a.substring(b);
		var g = e + f + d;
		if (c.data("high-res") && Mozilla.ImageHelper.isHighDpi()) {
			g = Mozilla.ImageHelper.convertUrlToHighRes(g)
		}
		this.src = g;
		c.addClass(site.platform)
	})
};
